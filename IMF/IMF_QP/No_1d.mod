...
matlab;
T  = 12;
tt = 0:T;                                  % include t = 0 impact period
mk = @(v,T) [0; reshape(v(1:T),[],1)];     % prepend 0 baseline value

// (d) Foreign interest rate shock
stoch_simul(order=1, irf=12, irf_shocks=(eta_i_star), nograph) y c ii pie ec i_star e_level ca;

figure('Name','Q1(d) – Foreign i\_rate shock (1% ε_{i*})','Position',[100 100 900 900]);
