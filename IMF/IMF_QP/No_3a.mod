...
//------------------------
// 5) IRF per Q1 (12 periods) + figure 4×2
//------------------------

matlab;
T  = 12;
tt = 0:T;                                % x-axis labels: t=0..T
mk = @(v,T) [0; reshape(v(1:T),[],1)];   % prepend zero, ensure column

// (a) Output shock
stoch_simul(order=1, irf=12, irf_shocks=(eta_y_star), nograph) y c ii pie ec y_star e_level ca;
...
figure('Name','Negative Shock of \eta_{y^*} (-5%)');
