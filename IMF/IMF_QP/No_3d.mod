...
varexo eta_phi_hat eta_z eta_H eta_y_star eta_pie_star eta_i_star;
...
psi_e   = 0.80;   // reponse to exchange rate fluctuations, useless in this case
...
matlab;
T  = 12;
tt = 0:T;                                % show impact at t=0
mk = @(v,T) [0; reshape(v(1:T),[],1)];   % prepend 0, column shape

stoch_simul(order=1, irf=12, irf_shocks=(eta_y_star, eta_z), nograph) y c ii pie ec y_star e_level ca;

% --- Build negative IRFs (to show -5% y* and -3% preference) ---
...
% --- Linear combination (two negative shocks together) ---
...
figure('Name','Negative Shock eta_z (-0.03), fixed e_rate','NumberTitle','off');
...
figure('Name','Negative Shock eta_y* (-0.05), fixed e_rate','NumberTitle','off');
...
% Combined negative shocks
figure('Name','Combined (eta_y* -0.05, eta_z -0.03), fixed e_rate','NumberTitle','off');
