...
matlab;
T  = 12;
tt = 0:T;                                  % include impact period
mk = @(v,T) [0; reshape(v(1:T),[],1)];     % prepend 0 baseline

// (c) Risk premia shock
stoch_simul(order=1, irf=12, irf_shocks=(eta_phi_hat), nograph) y c ii pie ec phi_hat e_level ca;

figure('Name','Q1(c) – Risk premium shock (1% η_{φ̂})','Position',[100 100 900 900]);
subplot(4,2,1); plot(tt, mk(oo_.irfs.y_eta_phi_hat, T));       yline(0,'r','LineWidth',1); title('Output y_t');             grid on
subplot(4,2,2); plot(tt, mk(oo_.irfs.c_eta_phi_hat, T));       yline(0,'r','LineWidth',1); title('Consumption c_t');        grid on
subplot(4,2,3); plot(tt, mk(oo_.irfs.ii_eta_phi_hat, T));      yline(0,'r','LineWidth',1); title('Interest rate i_t');      grid on
subplot(4,2,4); plot(tt, mk(oo_.irfs.pie_eta_phi_hat, T));     yline(0,'r','LineWidth',1); title('CPI inflation \pi_t');    grid on
subplot(4,2,5); plot(tt, mk(oo_.irfs.ec_eta_phi_hat, T));      yline(0,'r','LineWidth',1); title('Depreciation e^c_t');     grid on
subplot(4,2,6); plot(tt, mk(oo_.irfs.phi_hat_eta_phi_hat, T)); yline(0,'r','LineWidth',1); title('Shocked var: φ̂_t');      grid on
subplot(4,2,7); plot(tt, mk(oo_.irfs.e_level_eta_phi_hat, T)); yline(0,'r','LineWidth',1); title('Nominal FX level');       grid on
subplot(4,2,8); plot(tt, mk(oo_.irfs.ca_eta_phi_hat, T));      yline(0,'r','LineWidth',1); title('Current account (y-c)');  grid on
